...
struct Layer{
	...
	Layer(int n, int input_imgsize, int batchsize) : 
		...
		Aof_concat(new Volume[batchsize]),
		...
		Aoe_concat(new Volume[batchsize]),
		...
		Aok1u_gradient(new Volume[num_of_features]),
		Aok2u_gradient(new Volume[num_of_features]),
		...
		Aok1u_back(new Volume[num_of_features*2]),
		...
		Aok_final_back(new Volume[num_of_features])
		{
			for (int i=0; i<batchsize; ++i){
				...
				Aof_concat[i]=Volume(num_of_features*2, padded_imgsize); // concatenation of Aof2d and Aofinu
				Aof1u[i]=Volume(num_of_features, padded_imgsize);
				Aof2u[i]=Volume(num_of_features, padded_imgsize);
				...
				Aoe_concat[i]=Volume(num_of_features*2, padded_imgsize);
				Aoe1u[i]=Volume(num_of_features, padded_imgsize);
				Aoe2u[i]=Volume(num_of_features, padded_imgsize);
			...
			for (int i=0; i<num_of_features; ++i){
				...
				Aok1u[i]=Volume(num_of_features*2, 3);
	...
	ArrOfVols Aofind, Aof1d, Aof2d, Aofinu, Aof_concat, Aof1u, Aof2u, Aof_final;	// Aof1d[num of images in the batch](num of features, x, y)
	ArrOfVols Aoeind, Aoe1d, Aoe2d, 		Aoe_concat, Aoe1u, Aoe2u, Aoe_final;	

	ArrOfVols Aok1d, 			Aok2d, 			Aok1u, 			Aok2u, 			Aok_uc, 		Aok_final; 			// Aok1d[num of kernels / ouput channels](depth / number of input features, x, y)
	ArrOfVols Aok1d_gradient, 	Aok2d_gradient,	Aok1u_gradient,	Aok2u_gradient,					Aok_final_gradient;
	ArrOfVols Aok1d_back, 		Aok2d_back, 	Aok1u_back, 	Aok2u_back, 					Aok_final_back;
